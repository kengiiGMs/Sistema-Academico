...
const JanelaModalEditar = ({ abrir, fechar, nome, id, endereco, curso, telefone, email }) => {
    ...
    return (
        <JanelaModal fechar={fechar} abrir={abrir} funcaoAoEnviarFormulario={efetuarEdicao} carregando={carregando} textoBotao='Editar' >
            ...
            <TextField id='nome' label='Insira o Nome' variant='filled' value={nomeEditar} onChange={(e) => { setNomeEditar(e.target.value) }} required sx={{ marginBlock: '5px' }} />
            <TextField id='endereco' label='Insira o Endereço' variant='filled' value={enderecoEditar} onChange={(e) => { setEnderecoEditar(e.target.value) }} required sx={{ marginBlock: '5px' }} />
            <TextField id='curso' label='Insira o Curso' variant='filled' value={cursoEditar} onChange={(e) => { setCursoEditar(e.target.value) }} required sx={{ marginBlock: '5px' }} />
            <TextField id='telefone' label='Insira o Telefone' variant='filled' value={telefoneEditar} onChange={(e) => { setTelefoneEditar(e.target.value) }} required sx={{ marginBlock: '5px' }} />
            <TextField id='email' label='Insira o Email' variant='filled' value={emailEditar} onChange={(e) => { setEmailEditar(e.target.value) }} required sx={{ marginBlock: '5px' }} />
        </JanelaModal>
    )
}
